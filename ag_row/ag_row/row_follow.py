...
from crd_utils.unetwsess import * #U-Net model inference functions
from crd_utils.data import * # Data Loader for U-Net
from crd_utils.triangle_scan import * #CRD Post processing
from crd_utils.exitman import * # Exit Manoeuvre
from crd_utils.utils import * #Miscellaneous (math functions and etc)

ROBOT = "Husky" # [Husky, Mark1, Hunter, Leo, HuskySim]
CAMERA = "D435i"# [D435i, Leo]
    ...
    def run(self):
        ...
        while not rclpy.ok():
            if self.rgb_image is not None and self.cr_mask is not None and self.depth_image is not None and self.camera_info is not None:
                ...
                #self.nav_diagnose()
            self.node.get_clock().sleep_for(rate.period)
        #rospy.spin()
...
if __name__ == '__main__':
    ...
    model_path = rospkg.RosPack().get_path("ag_row") + "/models/unet.hdf5"
    model.load_weights(model_path)
    print("U-Net Model Loaded Successfully")

            
    realsense_cmd = ['roslaunch', 'realsense2_camera', 'rs_camera.launch', 'enable_stereo:=false']
    process_rs = subprocess.Popen(realsense_cmd)
